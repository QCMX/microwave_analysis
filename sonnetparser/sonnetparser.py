# -*- coding: utf-8 -*-
"""
Functions to load output files from the Sonnet EM software.

It is possible to save simulation results as project output files with
S, Y or Z parameters (here parsed from the CSV format), or save any kind
of curve from the graph tool. The resulting CSV formats have
non-conventional containing the simulation parameters.

Functions here try to parse as much of the meta data as reasonable.

TODO: return in a more nice format than dicts.

See `output_formats.md` for example data files.
"""
...
def load_sonnet_output_csv(fname):
    ...
    Returns a list of dicts, with one dict per parameter combination.
    The dicts have the fields

    - `data` the simulation data as numpy array
    - `columns` the column headers as a list of strings
    - `params` dict with parameters as keys
    - `termination` the port impedances
    - `comments` (string)
    """
...
def load_sonnet_output_Sparam(fname, pnames=[], columnfmt=None):
    ...
    pnames : list of strings
        Can be a list of parameter names to determine the parameters to be
        tabulated if not all parameters should become axes of the output.
    columnfmt : string
        Is a hint for the column format, needed if no column header
        is present in the file. Use `'MAG-ANG'`, `'DB-ANG'`, or `'RE-IM'`.

    Returns
    -------
    params: dict
        Parameter values like :code:`{pname: [values], pname2: [values], ...}`
    freq : numpy.ndarray
        Frequency points
    data : numpy.ndarray
        Data with shape (param1, param2, ..., frequencies, nports, nports).
    """
    ...
    for chunk in chunks:
        ...
        if columnfmt is None:
            if 'columns' in chunk and chunk['columns'] is not None:
...
def load_sonnet_graph_csv(fname):
    ...
    - `path` sonnet file path
    - `settings` some simulation settings
    - `params` dict with parameters as keys
    - `header` column header
    - `data` 2-N numpy array: x axis is data[0], y axis is data[1]

    Doesn't assemble results into array, because each line may have different
    number of points, since Sonnet mixes new and cached simulation results.
    """
    with open(fname) as f:
        chunks = []
        for line in f:
